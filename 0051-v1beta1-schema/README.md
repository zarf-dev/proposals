...
- `.components.[x].repos` will be renamed to `.components.[x].repositories` and will move from a list of strings to a list of objects. The `Repository` object has a required `url` and an optional `ref` sub-object with `tag`, `branch`, and `commit` fields. `ref` is optional because a mirror may clone the entire repository; when `ref` is omitted, the whole repository is mirrored.  [Git URL and Ref Parsing](#git-url-and-ref-parsing).
