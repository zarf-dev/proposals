...
Additionally, introduce a command for developers to preview a package during implementation. This will be achieved through the `zarf dev inspect` command, with three subcommands: `definition`, `sbom` and `values-files`. These commands will accept directory containing a `zarf.yaml` file. 
